...
// Data hari libur nasional dan cuti bersama tahun 2025
final List<Map<String, String>> nationalHolidaysJson = [
  ...
  {"tanggal": "2025-08-18", "keterangan": "Libur Nasional Kemerdekaan Republik Indonesia"},
...
// Mengubah data JSON menjadi List<DateTime> dengan DateFormat
// Gunakan format 'yyyy-MM-d' yang lebih fleksibel
final List<DateTime> parsedHolidays = nationalHolidaysJson
    .map((holiday) => DateFormat('yyyy-MM-d').parse(holiday['tanggal']!))
    .toList();
...
// Fungsi baru untuk mendapatkan keterangan hari libur
String? getHolidayDescription(DateTime date) {
  for (var holiday in nationalHolidaysJson) {
    // Gunakan isSameDay untuk membandingkan tanggal
    if (isSameDay(DateFormat('yyyy-MM-d').parse(holiday['tanggal']!), date)) {
